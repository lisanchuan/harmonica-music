...
async function getHarmonicaSongs(): Promise<Song[]> {
  const base = process.cwd();
  const songsFile = path.join(base, 'data', 'songs.json');
  return JSON.parse(fs.readFileSync(songsFile, 'utf-8')) as Song[];
}

export default async function HarmonicaPage() {
  const songs = await getHarmonicaSongs();
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800">
      ...
      <SongList songs={songs} basePath="/harmonica" />
